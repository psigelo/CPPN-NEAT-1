...
Genetic_Encoding Population::mutation_node(Genetic_Encoding organism)
{
	...
	organism.add_node(node, row , HIDDEN, round(rand() % FUNCTION_NUM));
...
{	
	...
	{
		if(!flag_in && row_node_initial_in == row_orderer_list[i]) 
		{  
			row_position_in = i; 
			flag_in = true; 
		}

		if(!flag_out && row_node_initial_out == row_orderer_list[i])
		{  
			row_position_out = i; 
			flag_out = true;
		}
...
Genetic_Encoding Population::mutation_connection(Genetic_Encoding organism)
{
	...
	while(true)
	{
		...
		while(true)
		{
			...
			for (int i = 0; i < (int)row_orderer_list.size(); ++i)
			{
				if(organism.Lnode_genes[node_in].row == row_orderer_list[i])
				{   
					row_position_in = i; 
					flag_in = true;
				}

				if(organism.Lnode_genes[node_out].row == row_orderer_list[i])
				{ 	
					row_position_out = i; 
					flag_out = true;
				}
...
void Population::epoch()
{
	...
	//=============================================
	/*static int mr_counter = 0;
	cout << endl << "epoch " << mr_counter << " -------------------------- " << endl;
	mr_counter++;*/
	//=============================================
	...
	for (int i = 0; i < (int)current_niches.size(); ++i)
	{
		if(current_niches[i].exist)
		{
			...
			//cout << "POPULATION_MAX [[" << POPULATION_MAX << "]] * ( current_niches[" << i << "].total_fitness [[" << current_niches[i].total_fitness << "]] / current_niches[" << i << "].organism_position.size() [[" << current_niches[i].organism_position.size() << "]] ) / total_shared_fitness_population [[" << total_shared_fitness_population << "]] -> " << POPULATION_MAX * (current_niches[i].total_fitness / current_niches[i].organism_position.size()) / total_shared_fitness_population << endl; 
